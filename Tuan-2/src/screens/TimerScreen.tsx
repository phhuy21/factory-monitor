...
const TimerScreen: React.FC = () => {
  ...
  // Đồng bộ 2 chiều: cập nhật picker khi ESP32 gửi cấu hình hoặc chuyển node
  React.useEffect(() => {
    if (!hasChanges) {
      setOnHour(currentConfig.onHour);
      setOnMinute(currentConfig.onMinute);
      setOffHour(currentConfig.offHour);
      setOffMinute(currentConfig.offMinute);
    }
  }, [
    currentConfig.onHour,
    currentConfig.onMinute,
    currentConfig.offHour,
    currentConfig.offMinute,
    selectedNodeId,
    hasChanges,
  ]);
  ...
  const handleSave = useCallback(() => {
    ...
    const thresholds = node?.thresholds ?? { tempMax: 35, tempMin: 20, humMax: 80, gasMax: 2000 };
    firebaseService.saveConfig(selectedNodeId, thresholds, config);
